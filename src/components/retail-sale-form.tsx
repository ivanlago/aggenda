  ...
  return (
    <ActionForm action={registerRetailSale} successMessage="Venda registrada com sucesso." onSuccess={() => { setCart([]); setPayments([{ id: Date.now(), method: "cash", amount: "" }]); }} className="grid gap-5 xl:grid-cols-[1.25fr_0.75fr] xl:items-start">
      ...
      <aside className="panel form-stack xl:sticky xl:top-5">
        ...
        <label className="flex items-center gap-2 rounded-xl border px-3 py-3 text-sm font-bold"><input name="received" type="checkbox" defaultChecked required={cart.some((item) => variantsById.get(item.variantId)?.kind === "package" || item.variantId.startsWith("appointment:"))} /> Pagamento recebido</label>
