...
export function RetailQuoteForm({ catalog, clients, attendanceId, initialClientId, canDiscount, defaultValidity }: {
  ...
  try {
    const shares = allocateOrderDiscount(items.map((item) => (catalog.find((entry) => entry.id === item.variantId)?.priceInCents ?? 0) * item.quantity - item.discountInCents), discountType, canDiscount ? discountValue : "");
    discountedItems = items.map((item, index) => ({ ...item, discountInCents: item.discountInCents + shares[index] }));
  ...
  const discount = discountedItems.reduce((sum, item) => sum + item.discountInCents, 0);
  ...
  return <ActionForm action={saveRetailQuote} successMessage="Orçamento salvo. Disponível em Venda e orçamento." onSuccess={() => { setItems([]); setId(""); setDiscountValue(""); }} className="grid gap-5 xl:grid-cols-[1.25fr_0.75fr] xl:items-start">
    <input type="hidden" name="id" value={id} />
    <input type="hidden" name="items" value={discountError ? "[]" : JSON.stringify(discountedItems)} />
    ...
    <section className="panel form-stack xl:sticky xl:top-5">
      ...
      {attendanceId ? <input type="hidden" name="clientId" value={clientId} /> : <ClientSearch clients={clients} value={clientId} onChange={setClientId} />}
      {canDiscount && <OrderDiscountField type={discountType} value={discountValue} onTypeChange={setDiscountType} onValueChange={setDiscountValue} error={discountError} />}
      ...
      <button className="primary-button w-fit" disabled={!items.length || Boolean(discountError)}>Salvar e abrir orçamento em PDF</button>
    </section>
  </ActionForm>;
}
