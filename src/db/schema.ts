...
export const retailSaleItems = pgTable("retail_sale_items", {
  ...
  variantId: uuid("variant_id").references(() => retailProductVariants.id, { onDelete: "restrict" }),
  inventoryProductId: uuid("inventory_product_id").references(() => inventoryProducts.id, { onDelete: "restrict" }),
...
export const clientPackages = pgTable(
  "client_packages",
  {
    ...
    retailSaleId: uuid("retail_sale_id").references((): import("drizzle-orm/pg-core").AnyPgColumn => retailSales.id, { onDelete: "restrict" }),
    notes: text("notes"),
    createdAt: timestamp("created_at").defaultNow().notNull(),
    updatedAt: timestamp("updated_at").defaultNow().notNull(),
  },
