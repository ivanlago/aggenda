...
export function CertificateComposer({ templateId, clients, professionals }: {
  ...
  return <>
    ...
    <select className="field" name="professionalId" required value={professionalId} onChange={(event) => setProfessionalId(event.target.value)}><option value="" disabled>Médico emissor</option>{professionals.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select>
    ...
    <button type="button" className="primary-button w-fit" disabled={!valid} onClick={() => setPreviewOpen(true)}>Visualizar atestado</button>
