...
import { Download, Mail, MessageCircle, Plus, ShieldCheck, Trash2, X } from "lucide-react";
...
type InitialPrescription = { clientId?: string; professionalId?: string; kind?: string; observations?: string; medications?: Omit<Medication, "id">[] };
...
export function PrescriptionComposer({ templateId, organizationName, clients, professionals, initial }: { templateId: string; organizationName: string; clients: Option[]; professionals: Option[]; initial?: InitialPrescription | null }) {
  ...
  const [medications, setMedications] = useState<Medication[]>(initial?.medications?.length ? initial.medications.map((item, index) => ({ ...item, id: index + 1 })) : [emptyMedication(1)]);
  ...
  ].filter(Boolean).join("\n")).join("\n\n")}${observations.trim() ? `\n\nObservações gerais:\n${observations.trim()}` : ""}\n\nData: {{data}}.`, [medications, observations]);

  function update(id: number, field: keyof Medication, value: string) {
    setMedications((current) => current.map((item) => item.id === id ? { ...item, [field]: value } : item));
  }

  function selectMedication(id: number, item: TussItem) {
    setMedications((current) => current.map((medication) => medication.id === id ? { ...medication, name: item.name, presentation: item.presentation ?? "", tussCode: item.code } : medication));
  }
  ...
  return <>
    ...
    <input type="hidden" name="prescriptionData" value={JSON.stringify({ kind, medications: medications.map((item) => ({ name: item.name, presentation: item.presentation, route: item.route, dosage: item.dosage, quantity: item.quantity, notes: item.notes, tussCode: item.tussCode })), observations })} />
    ...
    <div className="grid gap-3">
      {medications.map((medication, index) => <div className="rounded-2xl border bg-slate-50/60 p-4" key={medication.id}>
        <div className="mb-3 flex items-center justify-between"><p className="font-extrabold">Medicamento {index + 1}</p>{medications.length > 1 && <button type="button" className="rounded-lg p-2 text-red-700 hover:bg-red-50" onClick={() => setMedications((current) => current.filter((item) => item.id !== medication.id))} aria-label={`Remover medicamento ${index + 1}`}><Trash2 className="size-4" /></button>}</div>
        <div className="grid gap-3 md:grid-cols-2">
          <div className="md:col-span-2"><TussAutocomplete table="20" label="Buscar medicamento" nameField={`medication-${medication.id}`} onSelect={(item) => selectMedication(medication.id, item)} /></div>
          <label className="grid gap-1 text-sm font-bold">Medicamento<input className="field" value={medication.name} onChange={(event) => update(medication.id, "name", event.target.value)} required placeholder="Nome do medicamento" /></label>
          <label className="grid gap-1 text-sm font-bold">Apresentação<input className="field" value={medication.presentation} onChange={(event) => update(medication.id, "presentation", event.target.value)} placeholder="Ex.: 20 mg, caixa com 30" /></label>
          <label className="grid gap-1 text-sm font-bold">Via de uso<select className="field" value={medication.route} onChange={(event) => update(medication.id, "route", event.target.value)}><option>Uso oral</option><option>Uso tópico</option><option>Uso subcutâneo</option><option>Uso intramuscular</option><option>Uso intravenoso</option><option>Uso inalatório</option><option>Uso oftálmico</option><option>Uso otológico</option><option>Outro</option></select></label>
          <label className="grid gap-1 text-sm font-bold">Quantidade (opcional)<input className="field" value={medication.quantity} onChange={(event) => update(medication.id, "quantity", event.target.value)} placeholder="Ex.: 1 caixa" /></label>
          <label className="grid gap-1 text-sm font-bold md:col-span-2">Posologia<input className="field" value={medication.dosage} onChange={(event) => update(medication.id, "dosage", event.target.value)} required placeholder="Ex.: tomar 1 comprimido a cada 8 horas por 7 dias" /></label>
          <label className="grid gap-1 text-sm font-bold md:col-span-2">Orientações específicas<input className="field" value={medication.notes} onChange={(event) => update(medication.id, "notes", event.target.value)} placeholder="Opcional" /></label>
        </div>
      </div>)}
      <button type="button" className="secondary-button w-fit" onClick={() => setMedications((current) => [...current, emptyMedication(Math.max(...current.map((item) => item.id)) + 1)])}><Plus className="mr-2 size-4" />Adicionar medicamento</button>
    </div>
    ...
    <button type="button" className="primary-button w-fit" onClick={(event) => { const form = event.currentTarget.closest("form"); if (form?.reportValidity()) setPreviewOpen(true); }}>Visualizar receita</button>
    ...
    {previewOpen ? <div className="fixed inset-0 z-50 grid place-items-center overflow-y-auto bg-slate-950/60 p-4" role="dialog" aria-modal="true" aria-labelledby="prescription-preview-title">
      <div className="my-6 w-full max-w-3xl rounded-3xl bg-white p-5 shadow-2xl sm:p-8">
        ...
        <div className="my-6 rounded-xl border bg-white p-6 shadow-sm sm:p-10">
          ...
          <div className="mt-8 border-t pt-4"><p className="font-bold">{selectedProfessional?.name}</p><p className="text-sm text-muted">Data de emissão: {new Date().toLocaleDateString("pt-BR")}</p></div>
        </div>
