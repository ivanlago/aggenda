...
export default async function ClientPortalPage({ params, searchParams }: { params: Promise<{ slug: string }>; searchParams: Promise<{ erro?: string }> }) {
  ...
  if (!identity) return <main className="min-h-screen p-4 sm:p-8" style={{ background: `linear-gradient(145deg, ${organization.brandColor}18, #f3f5f1 55%)` }}><div className="mx-auto grid max-w-5xl gap-6"><CompanyHero organization={organization} /><section className="panel mx-auto w-full max-w-xl shadow-xl shadow-brand/5"><div className="text-center"><span className="mx-auto grid size-12 place-items-center rounded-2xl bg-accent text-brand-dark"><CalendarCheck className="size-6" /></span><p className="mt-4 text-xs font-extrabold uppercase tracking-widest text-brand">Acesso seguro</p><h1 className="mt-1 text-3xl font-extrabold">Sua área do cliente</h1><p className="mx-auto mt-2 max-w-md text-sm leading-6 text-muted">Identifique-se pelo e-mail ou celular cadastrado. Seus agendamentos só serão exibidos após a confirmação da sua identidade.</p></div><PortalLogin slug={slug} hasChallenge={Boolean(cookieStore.get(CLIENT_CHALLENGE_COOKIE))} expiredLink={query.erro === "link-expirado"} /><p className="mt-5 text-center text-xs leading-5 text-muted">Se seus dados estiverem desatualizados, entre em contato com a empresa para proteger seu cadastro.</p></section><CompanyFooter organization={organization} /></div></main>;
  ...
  return <main className="min-h-screen p-4 sm:p-8" style={{ background: `linear-gradient(145deg, ${organization.brandColor}18, #f3f5f1 55%)` }}><div className="mx-auto grid max-w-5xl gap-6">
    ...
    <section className="panel"><div className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-sm font-bold text-brand">Olá, {identity.name}</p><h1 className="text-3xl font-extrabold">Seus agendamentos</h1><div className="mt-2 flex flex-wrap gap-4 text-sm text-muted">{identity.email && <span className="flex items-center gap-1"><Mail className="size-4" />{identity.email}</span>}{identity.phone && <span className="flex items-center gap-1"><Phone className="size-4" />{identity.phone}</span>}</div></div><PortalLogout slug={slug} /></div><div className="mt-5 border-t pt-5"><a className="primary-button inline-flex" href="#novo-agendamento">Marcar novo horário</a></div></section>
    ...
    <section id="novo-agendamento" className="panel"><p className="text-xs font-extrabold uppercase tracking-widest text-brand">Agendamento guiado</p><h2 className="mt-1 text-2xl font-extrabold">Marcar novo horário</h2><p className="mt-2 text-sm text-muted">Mostraremos somente profissionais habilitados e horários livres para a sua escolha.</p><PortalBooking slug={slug} services={bookableServices} professionals={professionalCatalog} timezone={organization.timezone} horizonDays={organization.bookingHorizonDays} hasUpcoming={upcoming.length > 0} /></section>
...
function CompanyFooter({ organization }: { organization: typeof organizations.$inferSelect }) {
  ...
  const phones = [...new Set([organization.phone, organization.publicWhatsapp].filter(Boolean))] as string[];
