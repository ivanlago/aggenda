        ...
        try {
          if (input.entityType === "clients") {
            ...
            if (found) {
              ...
              existingClients.push({ id: created.id, organizationId: organization.id, name, phone: normalizedPhone || null, email, birthDate, gender, address: null, postalCode: null, maritalStatus: null, cpf: null, profession: null, notes: text(row.notes) || null, imageUrl: null, imagePublicId: null, createdAt: new Date(), updatedAt: new Date() });
            ...
            const values = { name, description: text(row.description) || null, tussCode: null, tussTable: null, tussName: null, shortName: null, preparation: null, durationMinutes, priceInCents: money(row.price), estimatedCostInCents: 0, depositType: "none", depositValue: 0, depositExpirationMinutes: 30, isActive: truthy(row.isActive, true), requiresProfessional: truthy(row.requiresProfessional, true), updatedAt: new Date() };
            if (found) {
