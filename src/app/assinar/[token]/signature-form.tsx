...
export function SignatureForm({ token, requireResponses = false }: { token: string; requireResponses?: boolean }) {
  ...
  return <form action={action} className="mt-7 grid gap-4">
    ...
    {requireResponses && <label className="grid gap-2 text-sm font-bold">Respostas da anamnese<textarea className="field min-h-48" name="signerResponses" required maxLength={20000} placeholder="Responda às perguntas apresentadas no documento. Identifique cada resposta com o número ou título correspondente." /></label>}
    <label className="grid gap-2 text-sm font-bold">Código recebido por e-mail<input className="field" name="verificationCode" inputMode="numeric" autoComplete="one-time-code" pattern="[0-9]{6}" maxLength={6} required placeholder="000000" /></label>
