...
export default async function ClientPortalPage({ params, searchParams }: { params: Promise<{ slug: string }>; searchParams: Promise<{ erro?: string }> }) {
  ...
  if (!identity) return <main className="grid min-h-screen place-items-center p-5" style={{ background: `linear-gradient(145deg, ${organization.brandColor}18, #f3f5f1 55%)` }}><section className="panel w-full max-w-md shadow-xl shadow-brand/5"><Header name={organization.name} /><h1 className="mt-6 text-2xl font-extrabold">Área do cliente</h1><p className="mt-2 text-sm leading-6 text-muted">Identifique-se pelo e-mail ou celular cadastrado. Enviaremos ao seu e-mail um link seguro e um código temporário.</p><PortalLogin slug={slug} hasChallenge={Boolean(cookieStore.get(CLIENT_CHALLENGE_COOKIE))} expiredLink={query.erro === "link-expirado"} /><p className="mt-5 text-center text-xs leading-5 text-muted">Se seus dados estiverem desatualizados, entre em contato com a clínica para proteger seu cadastro.</p></section></main>;
