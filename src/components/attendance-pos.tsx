...
export async function AttendancePos({ appointmentId }: { appointmentId: string }) {
  ...
  return <section id="pdv" className="mt-5 scroll-mt-6"><h2 className="mb-3 text-xl font-extrabold">PDV e pagamentos</h2>
    ...
    {canSell && <details className="mb-4 rounded-2xl border bg-white p-4"><summary className="cursor-pointer text-lg font-extrabold">Vender produtos neste atendimento</summary><div className="mt-4"><RetailSaleForm attendanceId={appointment.id} initialClientId={client.id} clients={[client]} variants={variants} canDiscount={hasOrganizationPermission(organization.role, "sales.discount")} /></div></details>}
