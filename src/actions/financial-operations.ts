...
export async function createCommissionRule(data: FormData) {
  ...
  await db.insert(commissionRules).values({ organizationId: organization.id, professionalId, serviceId, calculationType, value: calculationType === "percentage" ? Math.round(raw * 100) : Math.round(raw * 100) }); revalidatePath("/financeiro/operacoes");
}
...
export async function generateCommissions(data: FormData) {
  ...
  await writeAuditLog({ organizationId: organization.id, userId: session.user.id, action: "generate", entityType: "commission_entries", details: { competence, generated } }); revalidatePath("/financeiro/operacoes");
}

export async function markCommissionPaid(data: FormData) { const { organization } = await requireOrganization(); assertOrganizationPermission(organization.role, "finance.manage"); const id = value(data, "id"); await db.update(commissionEntries).set({ status: "paid", paidAt: new Date(), updatedAt: new Date() }).where(and(eq(commissionEntries.id, id), eq(commissionEntries.organizationId, organization.id))); revalidatePath("/financeiro/operacoes"); }
