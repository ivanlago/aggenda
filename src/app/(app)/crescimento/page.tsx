...
export default async function GrowthPage() {
  ...
  return <div className="page-wrap">
    ...
    <section className="mt-5 grid gap-5 xl:grid-cols-2">
      <ActionForm action={createMembership} successMessage="Assinatura criada no Asaas." className="panel form-stack"><div><h2 className="text-lg font-extrabold">Nova assinatura mensal</h2><p className="mt-1 text-sm text-muted">O pacote define os benefícios; o Asaas da empresa cobra mensalmente.</p></div>{!asaas.length && <p className="rounded-xl bg-amber-50 p-3 text-sm font-bold text-amber-800">Conecte a conta Asaas antes de ativar mensalidades. <Link className="underline" href="/financeiro/operacoes#cobrancas">Configurar agora</Link></p>}<select className="field" name="clientId" required defaultValue=""><option value="" disabled>Selecione o paciente</option>{clientRows.map((client) => <option key={client.id} value={client.id}>{client.name}</option>)}</select><select className="field" name="packageId" required defaultValue=""><option value="" disabled>Selecione os benefícios</option>{packageRows.map((bundle) => <option key={bundle.id} value={bundle.id}>{bundle.name}</option>)}</select><input className="field" name="document" required inputMode="numeric" placeholder="CPF/CNPJ do pagador" /><div className="grid gap-3 sm:grid-cols-2"><input className="field" name="monthlyPrice" required inputMode="decimal" placeholder="Mensalidade" /><input className="field" name="billingDay" type="number" min="1" max="28" defaultValue="5" aria-label="Dia da cobrança" /></div><button className="primary-button" disabled={!asaas.length}>Ativar cobrança recorrente</button></ActionForm>
