...
export async function moveInventory(data: FormData) { const { session, organization } = await requireOrganization(); assertOrganizationPermission(organization.role, "inventory.manage"); const productId = text(data, "productId"); const type = text(data, "type"); const entered = quantity(data, "quantity"); const notes = text(data, "notes"); if (!entered || !["entry", "exit", "consumption"].includes(type)) throw new Error("Informe tipo e quantidade."); if (type !== "consumption" && notes.length < 3) throw new Error("Informe o motivo da movimentação."); await db.transaction(async (tx) => { await tx.execute(sql`select id from inventory_products where id = ${productId} for update`); const [product] = await tx.select().from(inventoryProducts).where(and(eq(inventoryProducts.id, productId), eq(inventoryProducts.organizationId, organization.id))).limit(1); if (!product) throw new Error("Produto não encontrado."); const delta = type === "entry" ? entered : -entered; const balance = product.currentQuantityMillis + delta; if (balance < 0) throw new Error("A movimentação deixaria o estoque negativo."); await tx.update(inventoryProducts).set({ currentQuantityMillis: balance, consumptionQuantityMillis: type === "consumption" ? product.consumptionQuantityMillis + entered : product.consumptionQuantityMillis, updatedAt: new Date() }).where(eq(inventoryProducts.id, productId)); await tx.insert(inventoryMovements).values({ organizationId: organization.id, productId, type, quantityMillis: delta, balanceAfterMillis: balance, notes: type === "consumption" ? "Transferência do estoque para consumo na loja" : notes, createdByUserId: session.user.id }); }); revalidatePath("/estoque"); }
