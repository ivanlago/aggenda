...
export async function moveInventory(data: FormData) {
  const { session, organization } = await requireOrganization();
  assertOrganizationPermission(organization.role, "inventory.manage");
  const productId = text(data, "productId");
  const type = text(data, "type");
  const entered = quantity(data, "quantity");
  const notes = text(data, "notes");
  if (!entered || !["entry", "exit", "consumption", "usage"].includes(type)) throw new Error("Informe tipo e quantidade.");
  if (!["consumption", "usage"].includes(type) && notes.length < 3) throw new Error("Informe o motivo da movimentação.");
  await db.transaction(async (tx) => {
    await tx.execute(sql`select id from inventory_products where id = ${productId} for update`);
    const [product] = await tx.select({
      id: inventoryProducts.id,
      saleBalance: inventoryProducts.currentQuantityMillis,
      ...
      .where(and(eq(inventoryProducts.id, productId), eq(inventoryProducts.organizationId, organization.id)))
      .limit(1);
    if (!product) throw new Error("Produto não encontrado.");

    let saleBalance = product.saleBalance;
    let consumptionBalance = product.consumptionBalance;
    let balanceAfter = saleBalance;
    let movementNotes = notes;
    const delta = type === "entry" ? entered : -entered;

    if (type === "consumption") {
      if (saleBalance < entered) throw new Error("Saldo insuficiente no estoque de venda.");
      saleBalance -= entered;
      consumptionBalance += entered;
      balanceAfter = consumptionBalance;
      movementNotes = "Transferido do estoque de venda para consumo";
      if (saleBalance === 0) await tx.update(retailProductVariants).set({ isForSale: false, updatedAt: new Date() }).where(eq(retailProductVariants.id, product.variantId));
    } else if (type === "usage") {
      if (consumptionBalance < entered) throw new Error("Saldo insuficiente no estoque de consumo.");
      consumptionBalance -= entered;
      balanceAfter = consumptionBalance;
      movementNotes = "Retirada para uso interno";
    } else if (type === "entry" && !product.isForSale) {
      consumptionBalance += entered;
      balanceAfter = consumptionBalance;
    } else if (type === "exit" && !product.isForSale) {
      consumptionBalance -= entered;
      balanceAfter = consumptionBalance;
      if (consumptionBalance < 0) throw new Error("A movimentação deixaria o estoque de consumo negativo.");
    } else {
      saleBalance += delta;
      balanceAfter = saleBalance;
      if (saleBalance < 0) throw new Error("A movimentação deixaria o estoque de venda negativo.");
    }

    await tx.update(inventoryProducts).set({ currentQuantityMillis: saleBalance, consumptionQuantityMillis: consumptionBalance, updatedAt: new Date() }).where(eq(inventoryProducts.id, productId));
    await tx.insert(inventoryMovements).values({ organizationId: organization.id, productId, type, quantityMillis: delta, balanceAfterMillis: balanceAfter, notes: movementNotes, createdByUserId: session.user.id });
  });
