...
export function RetailSaleForm({ variants, clients }: { variants: Variant[]; clients: Client[] }) {
  ...
  const discountInCents = Math.max(0, Math.round((Number(discount.replace(",", ".")) || 0) * 100));
  ...
  return (
    <ActionForm action={registerRetailSale} successMessage="Venda registrada e estoque atualizado." onSuccess={() => { setCart([]); setDiscount("0"); }} className="grid gap-5 xl:grid-cols-[1.25fr_0.75fr] xl:items-start">
      ...
      <aside className="panel form-stack xl:sticky xl:top-5">
        ...
        <fieldset className="grid gap-2"><legend className="mb-1 text-sm font-bold">Forma de pagamento</legend><div className="grid grid-cols-3 gap-2">{[["cash", "Espécie"], ["card", "Cartões"], ["pix", "PIX"]].map(([value, label], index) => <label className="flex cursor-pointer items-center justify-center gap-2 rounded-xl border p-3 text-xs font-bold has-checked:border-brand has-checked:bg-brand/5" key={value}><input name="paymentMethod" type="radio" value={value} defaultChecked={index === 0} /> {label}</label>)}</div></fieldset>
        <label className="grid gap-1 text-sm font-bold">Desconto (R$)<input className="field" name="discount" inputMode="decimal" value={discount} onChange={(event) => setDiscount(event.target.value)} /></label>
        ...
        <div className="grid gap-2 rounded-2xl border p-3"><p className="text-sm font-extrabold">Enviar recibo</p><input className="field" name="receiptPhone" value={receiptPhone} onChange={(event) => setReceiptPhone(event.target.value)} placeholder="WhatsApp do comprador" /><input className="field" name="receiptEmail" type="email" value={receiptEmail} onChange={(event) => setReceiptEmail(event.target.value)} placeholder="E-mail do comprador" /><div className="flex flex-wrap gap-4"><label className="flex items-center gap-2 text-xs font-bold"><input type="checkbox" name="sendReceiptWhatsapp" /> WhatsApp</label><label className="flex items-center gap-2 text-xs font-bold"><input type="checkbox" name="sendReceiptEmail" /> E-mail</label></div><p className="text-xs text-muted">O recibo para impressora térmica será aberto ao concluir.</p></div>
        ...
        <button className="primary-button justify-center" disabled={!cart.length}>Concluir venda</button>
      </aside>
    </ActionForm>
  );
}
